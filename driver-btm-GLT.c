...
int opt_bitmain_glt_freq = 100;
int opt_bitmain_glt_voltage = 176;
int8_t opt_bitmain_glt_core_temp = 2;
...
void chain_inactive(int const which_chain)
{
    ...
    glt_write(fd, cmd_buf, CMD_LENTH);
}
...
void open_core_chain(unsigned int which_chain)
{
    ...
    struct work_glt null_work;

    memset(&null_work, 0, sizeof(struct work_glt));
    ...
    if(dev.chain_exist[which_chain] == 1)
    {
        ...
        for(which_core=0; which_core < 32; which_core++)
        {
            ...
            cgsleep_ms(5);
            glt_write(dev.dev_fd[which_chain], &null_work, WORK_INPUT_LENGTH_WITH_CRC);
            cgsleep_ms(5);
...
void calculate_hash_rate(void)
{
    ...
    for(which_chain = 0; which_chain < BITMAIN_MAX_CHAIN_NUM; which_chain++)
    {
        if(dev.chain_exist[which_chain])
        {
            if(g_HASH_RATE_reg_value_num[which_chain] == 0)
            {
                ...
                {
                    rate[which_chain] = 0;
                    suffix_string_glt(rate[which_chain], (char * )displayed_rate[which_chain], sizeof(displayed_rate[which_chain]), 6, true);
                }
                continue;
            }
            ...
            else
            {
                if(is_rt)
                {
                    ...
                    suffix_string_glt(rate[which_chain], (char * )displayed_rate[which_chain], sizeof(displayed_rate[which_chain]), 6, false);
                }
                else
                {
                    tmp_rate = 0;
    ...
    suffix_string_glt(tmp_rt_rate_all_chain, (char * )displayed_hash_rate, sizeof(displayed_hash_rate), 6, false);
}
...
int open_uart(struct bitmain_glt_info *info, uint8_t which_chain)
{
...
int uart_init(struct bitmain_glt_info *info,uint8_t which_chain, int baud)
{
    int i,ret;
    speed_t speed;
...
void tty_init_chain(uint8_t which_chain, struct bitmain_glt_info *info, int baud)
{
    int ret;
    ...
    if(info->chain_status[which_chain] == 1)
    {
        ...
        cgsleep_ms(50);
        struct bitmian_glt_info_with_index info_with_index;
        info_with_index.info = info;
        info_with_index.chain_index = which_chain;
        ret = thr_info_create(&info->uart_tx_t[which_chain], NULL, glt_fill_work, (void *)(&info_with_index));
        cgsleep_ms(200);
        if(unlikely(ret != 0))
        {
...
void tty_init(struct bitmain_glt_info *info, int baud)
{
...
int glt_write(int fd, const void *buf, size_t bufLen)
{
    size_t ret = 0;
...
int glt_read(int uart_fd, unsigned char *buf, size_t MAX_READ_BYTES)
{
...
void i2c_init(struct bitmain_glt_info *info)
{
    int which_chain;
...
void check_chain(struct bitmain_glt_info *info)
{
    int i, fd, ret;
...
void reset_chain(struct bitmain_glt_info *info, uint8_t chain)
{
    pthread_mutex_lock(&iic_mutex);
...
void recheck_asic_num(struct bitmain_glt_info *info, uint8_t chain)
{
...
int bitmain_glt_init(struct bitmain_glt_info *info)
{
    struct init_config config = info->glt_config;
...
void* bitmain_glt_reinit_chain(void * usrdata)
{
    pthread_detach(pthread_self());
    pthread_mutex_lock(&reinit_mutex);
    struct bitmian_glt_info_with_index *info_with_chain = (struct bitmian_glt_info_with_index *)usrdata;
    struct bitmain_glt_info *info = info_with_chain->info;
...
void suffix_string_glt(uint64_t val, char *buf, size_t bufsiz, int sigdigits,bool display)
{
...
void *bitmain_scanhash(void *arg)
{
    ...
    while(nonce_fifo.nonce_num)
    {
        ...
        {
            applog(LOG_ERR, "%s %d: work %02x not find error", bitmain_glt->drv->name, bitmain_glt->device_id, work_id);
        }
    crc_error:
...
{
    struct bitmain_glt_info *info = (struct bitmain_glt_info*)arg;
    ...
    struct bitmian_glt_info_with_index info_with_index[BITMAIN_MAX_CHAIN_NUM];
            ...
            for(which_chain=0; which_chain<BITMAIN_MAX_CHAIN_NUM; which_chain++)
            {
                if(dev.chain_exist[which_chain])
                {
                    ...
                    if((need_reinit[which_chain] || (each_chain_h_avg[which_chain]/1000000/10) < (double)(( ASIC_NUM_EACH_CHAIN * dev.frequency * dev.corenum * 0.80) / 13 ))&& (!status_error))
                    {
						...
						ret = pthread_create(&reinit_id[which_chain], NULL, bitmain_glt_reinit_chain, (void *)(&info_with_index[which_chain]));
#ifdef REINIT_LOG
                        if(ret != 0)
...
int create_bitmain_check_miner_status_pthread(struct bitmain_glt_info *info)
{
...
void *glt_fill_work(void *usrdata)
{
    pthread_detach(pthread_self());
    ...
    while(1 && !reiniting[chainid])
    {
        ...
        more_work:
            ...
            */
            ...
            glt_write(info->dev_fd[chainid], (uint8_t *)&workdata, WORK_INPUT_LENGTH_WITH_CRC);
            // cg_runlock(&info->update_lock);
...
void *get_asic_response(void* arg)
{
    ...
    while(start_recv[chainid])
    {
        ...
        len = glt_read(fd, receive_buf, ASIC_RETURN_DATA_LENGTH*90);
        for(i = 0; i < len; i++)
        {
...
static void bitmain_glt_detect(__maybe_unused bool hotplug)
{
    ...
    cgpu->threads = 1;
    cgpu->device_data = calloc(sizeof(struct bitmain_glt_info), 1);
...
static bool bitmain_glt_prepare(struct thr_info *thr)
{
    struct cgpu_info *bitmain_glt = thr->cgpu;
    struct bitmain_glt_info *info = bitmain_glt->device_data;
    ...
    struct init_config glt_config =
    {
        ...
        .temperature                = 80,
        .frequency                  = opt_bitmain_glt_freq,
        .voltage                    = {0x07,0x25},
    ...
    glt_config.crc = crc_itu_t(0xff,(uint8_t *)(&glt_config), sizeof(glt_config)-2);
    info->glt_config = glt_config;

    bitmain_glt_init(info);

    return true;
}


static int64_t bitmain_glt_scanhash(struct thr_info *thr)
{
    h = 0;
    pthread_t send_id;
...
static void bitmain_glt_update(struct cgpu_info *bitmain)
{
    int i = 0;
...
static void bitmain_glt_reinit_device(struct cgpu_info *bitmain)
{
    if(!status_error)
        system("/etc/init.d/cgminer.sh restart > /dev/null 2>&1 &");
}

static void get_bitmain_statline_before(char *buf, size_t bufsiz, struct cgpu_info *bitmain_glt)
{
    struct bitmain_glt_info *info = bitmain_glt->device_data;
}

static void bitmain_glt_shutdown(struct thr_info *thr)
{
...
struct device_drv bitmainA3_drv =
{
    ...
    .dname = "Bitmain_A3 (GLT Mode)",
    .name = "A3 (GLT Mode)",
    .drv_detect = bitmain_glt_detect,
    .thread_prepare = bitmain_glt_prepare,
    .hash_work = &hash_driver_work,
    .scanwork = bitmain_glt_scanhash,
    .update_work = bitmain_glt_update,
    .get_api_stats = bitmain_api_stats,
    .reinit_device = bitmain_glt_reinit_device,
    .get_statline_before = get_bitmain_statline_before,
    .thread_shutdown = bitmain_glt_shutdown,
};
