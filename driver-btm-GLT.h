#ifndef __DRIVER_BTM_GLT_H__

#define __DRIVER_BTM_GLT_H__
...
struct bitmian_glt_info_with_index
{
    struct bitmain_glt_info *info;
    uint8_t chain_index;
}__attribute__((packed, aligned(4)));

struct bitmain_glt_info
{
...
struct work_glt
{
    ...
    uint8_t work[WORK_DATA_INPUT_LENGTH];   // 0-75 bytes are glt work; 76-79 are start nonce field
...
struct work_buf
{
    ...
    uint8_t p_rd;
    uint8_t work_num;
    struct work_glt workdata[BITMAIN_MAX_CHAIN_NUM];
} __attribute__((packed, aligned(4)));
*/
